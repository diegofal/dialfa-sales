...
// Check if a certificate has the searched colada
function hasColada(certificate: CertificateResponse, colada: string): boolean {
  return certificate.coladas.some((c) => c.colada_number.toLowerCase() === colada.toLowerCase());
}

// Sort coladas putting the searched one first
function sortColadas(
  coladas: CertificateResponse['coladas'],
  searchedColada: string
): CertificateResponse['coladas'] {
  if (!searchedColada) return coladas;
  const lower = searchedColada.toLowerCase();
  return [...coladas].sort((a, b) => {
    const aMatch = a.colada_number.toLowerCase() === lower ? 0 : 1;
    const bMatch = b.colada_number.toLowerCase() === lower ? 0 : 1;
      ...
      <Table>
        ...
        <TableBody>
          {groupedCertificates.map((certificate) => {
            const isMatch = searchedColada ? hasColada(certificate, searchedColada) : false;
            const sortedColadas = sortColadas(certificate.coladas, searchedColada || '');
            return (
              <TableRow
                key={certificate.id}
                className={
              ...
              >
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {sortedColadas.slice(0, 3).map((colada) => {
                      const isSearched =
                        searchedColada &&
                        colada.colada_number.toLowerCase() === searchedColada.toLowerCase();
                      return (
                        <Badge
                          key={colada.id}
                          variant={isSearched ? 'default' : 'outline'}
                          className={isSearched ? 'text-xs font-semibold' : 'text-xs'}
                        >
                          {colada.colada_number}
                        </Badge>
                      );
                    })}
                    {sortedColadas.length > 3 && (
