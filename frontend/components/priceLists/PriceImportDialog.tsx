...
export function PriceImportDialog({
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsed: ImportPreviewItem[] = dataLines
          .map((line) => {
            ...
            const hasCostAndCif =
              article.costPrice &&
              article.costPrice > 0 &&
              article.cifPercentage &&
              article.cifPercentage > 0;
  ...
  const stats = {
    ...
    avgMarkup:
      csvData
        .flatMap(
          (i) => i.paymentTermPrices.map((p) => p.markup).filter((m) => m !== undefined) as number[]
        )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] !w-[1800px] !max-w-[98vw] flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle>Importar Precios desde CSV</DialogTitle>
          <DialogDescription>
            Sube un archivo CSV con las columnas: Codigo, NuevoDB (precio sin descuento). Los
            precios propuestos se guardarán automáticamente y persistirán aunque refresques la
            página.
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              ...
              <div className="grid grid-cols-5 gap-4">
                ...
                <div className="rounded-lg bg-green-50 p-3 dark:bg-green-950">
                  <div className="text-muted-foreground text-xs">Encontrados</div>
                  <div className="text-2xl font-bold text-green-600">{stats.found}</div>
                </div>
                <div className="rounded-lg bg-red-50 p-3 dark:bg-red-950">
                  <div className="text-muted-foreground text-xs">No Encontrados</div>
                ...
                <div className="rounded-lg bg-yellow-50 p-3 dark:bg-yellow-950">
                  <div className="text-muted-foreground text-xs">Sin Últ. Precio</div>
                ...
                <div className="rounded-lg bg-purple-50 p-3 dark:bg-purple-950">
                  <div className="text-muted-foreground text-xs">Markup Promedio</div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="w-[100px] text-right">Precio Actual</TableHead>
                        <TableHead className="w-[100px] text-right">Precio CSV</TableHead>
                        <TableHead className="w-[80px] text-right">% Cambio</TableHead>
                        <TableHead className="w-[90px] text-right">Últ. P. Compra</TableHead>
                        <TableHead className="w-[70px] text-right">CIF %</TableHead>
                        <TableHead className="w-[90px] text-right">Valor CIF</TableHead>

                        {/* Columnas dinámicas por payment term */}
                        {paymentTerms.map((pt) => (
                          <TableHead
                            key={pt.paymentTermId}
                            className="w-[150px] text-right"
                            colSpan={3}
                          >
                            <div className="border-border border-l pl-2 text-center font-bold">
                              {pt.paymentTermCode}
                            </div>
                            <div className="text-muted-foreground mt-1 grid grid-cols-3 gap-1 text-xs">
                              <div>Precio</div>
                              <div>
                                <Tooltip>
                                  ...
                                  <TooltipContent side="top" className="w-[280px]">
                                    <div className="space-y-2">
                                      ...
                                      <p className="text-xs">
                                        Fórmula: (Precio - CIF) / Precio × 100
                                      </p>
                                      <p className="text-xs">
                                        Indica qué % del precio de venta es ganancia.
                                      </p>
                                    </div>
                                  </TooltipContent>
                                </Tooltip>
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent side="top" className="w-[280px]">
                                    <div className="space-y-2">
                                      ...
                                      <p className="text-xs">
                                        Cuánto ganas por cada dólar invertido.
                                      </p>
                                    </div>
                                  </TooltipContent>
                                </Tooltip>
                      ...
                      {csvData.map((item, idx) => {
                        ...
                        return (
                          <TableRow key={idx}>
                            ...
                            {/* Columnas dinámicas por payment term */}
                            {paymentTerms.map((pt) => {
                              const ptPrice = item.paymentTermPrices.find(
                                (p) => p.paymentTermId === pt.paymentTermId
                              );
              ...
              <div className="flex justify-between border-t pt-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={handleConfirm} disabled={stats.found === 0 || isProcessing}>
