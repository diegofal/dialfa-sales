...
export function CategoryDialog({ isOpen, onClose, categoryId }: CategoryDialogProps) {
  ...
  useEffect(() => {
    if (isEditing && category) {
      ...
      const discountValue =
        typeof category.defaultDiscountPercent === 'number'
      ...
      <DialogContent size="lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="code" {...register('code')} placeholder="Ej: CAT001" />
            {errors.code && <p className="text-destructive text-sm">{errors.code.message}</p>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="name">Nombre *</Label>
            <Input id="name" {...register('name')} placeholder="Ej: Electrónica" />
          ...
          {paymentTerms && paymentTerms.length > 0 && (
            <div className="space-y-2 border-t pt-4">
              ...
              {isEditing && !categoryDiscounts && (
                <div className="py-2 text-sm text-gray-500">Cargando descuentos...</div>
              )}
              <div className="space-y-2">
                {paymentTerms.map((term) => {
                  ...
                  return (
                    <div key={term.id} className="flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground text-xs">({term.days} días)</span>
                    </div>
                  );
                })}
              </div>
