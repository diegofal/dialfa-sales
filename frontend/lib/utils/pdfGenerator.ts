...
export class PDFService {
  ...
  async generateInvoicePDF(invoice: InvoiceData, template: PrintTemplate): Promise<Buffer> {
    if (!invoice) throw new Error('Invoice data is required');
    if (!template) throw new Error('Print template is required');
    if (!template.pageSize) throw new Error('Template must define pageSize');
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.renderField(
          doc,
          template.fields.fecha,
          new Date(invoice.invoice_date).toLocaleDateString()
        );

        this.renderField(doc, template.fields.numeroFactura, invoice.invoice_number);
  ...
  private renderItems(doc: PDFKit.PDFDocument, items: PDFItem[], config: ItemsConfiguration): void {
      ...
      try {
        ...
        if (template.items && deliveryNote.delivery_note_items) {
          const items: PDFDeliveryNoteItem[] = deliveryNote.delivery_note_items.map((item) => ({
            quantity: item.quantity,
            articleDescription:
              item.articles?.description || item.description || item.article_description || '',
