...
async function updateCertificateColadas(
  ...
  try {
    ...
    console.error(`  ❌ Error actualizando: ${error instanceof Error ? error.message : 'Unknown error'}`);
    stats.errors.push({
      file: fileName,
  ...
  try {
    ...
    for (let i = 0; i < certificates.length; i++) {
      ...
      if (excelMapping.has(cert.file_name)) {
        ...
        if (needsUpdate) {
          console.log(`  📊 Coladas Excel: ${coladaNumbers.length > 0 ? coladaNumbers.join(', ') : 'ninguna'}`);
          console.log(`  📊 Coladas actuales: ${currentColadas.length > 0 ? currentColadas.join(', ') : 'ninguna'}`);
    ...
    console.log(`⏭️  Sin cambios/No en Excel:   ${stats.notInExcel + (stats.total - stats.updated - stats.notInExcel)}`);
